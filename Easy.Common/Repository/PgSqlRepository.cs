...
namespace Easy.Common.Repository
{
    ...
    /// Dapper读写数据仓储
    /// </summary>
    public class PgSqlRepository<T> : IRepository<T> where T : EntityBase
    {
        ...
        public void Update(T model, string tableIndex = "")
        {
            ...
            string sql = $"UPDATE public.{_tableName + tableIndex} SET {valueSql} WHERE Id = @Id AND Version = @Version";
        ...
        public PageResult<T> SearchPageResult(SearchBase search)
        {
            ...
            using (IDbConnection connection = new NpgsqlConnection(_connectionString))
            {
                ...
                int totalCount = connection.QuerySingleOrDefault<int>($"SELECT COUNT(*) FROM public.{_tableName + tableIndex} WHERE IsDel = '0' {timeSql}", param);
                ...
                string sql = $"SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY Id DESC) AS RowNumber,* FROM public.{_tableName + tableIndex} WHERE IsDel = '0'  {timeSql}) AS Temp WHERE Temp.RowNumber > @StartIndex AND Temp.RowNumber <= @EndIndex";
