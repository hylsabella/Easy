...
namespace Easy.Common.Repository
{
    ...
    /// 默认只读SqlMapper
    /// </summary>
    public static class DefaultReadonlySqlMapBuilder
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private static ISqlMapper _sqlMapper;

        static DefaultReadonlySqlMapBuilder()
        {
            try
            {
                ...
                _sqlMapper = builder.Configure(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "SqlMapForReadonly.config"));
            ...
            {
                logger.Error(ex, "初始化【ReadonlySqlMapperBuilder】失败");
